(async()=>{
      ...
      root.dataset.init='1';
      if(!window.LAKEGLASS_DATA){await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='data.js';s.onload=resolve;s.onerror=reject;document.head.appendChild(s);});}
      const {GLASS_COLORS,REGIONS,THICKNESS_NOTES,MARK_NOTES}=window.LAKEGLASS_DATA;
      ...
      function renderDiagnostic(){
        ...
        if(!config){group.hidden=true;state.diagnostic=null;state.diagnosticKey=null;return;}
        group.hidden=false;
        if(state.diagnosticKey!==config.key){state.diagnostic=null;state.diagnosticKey=config.key;}
      ...
      function specimenCode(){return `LM-${new Date().getFullYear()}-${String(Math.floor(Math.random()*900+100))}`;}
      ...
      function formScore(value){return ({curved:2,flat:3,rim:7,base:6,neck:6,unknown:1})[value]||1;}
      ...
      function renderResult(){
        ...
        const fs=formScore(state.form), hs=historyScore();
      ...
      const savedSpecimens=[
        {key:'aqua',id:'LM-042',name:'Pale aqua',color:'#8cc9c5',provenance:'Loyola Beach · Chicago',period:'Not enough evidence',rarity:'2 / 10',source:'Container glass',notes:'Thin curved wall, transparent aqua color, and a heavily frosted surface support a utility-container reading. Color alone does not establish an exact date.'},
        {key:'amber',id:'LM-041',name:'Amber',color:'#b97b48',provenance:'Osterman Beach · Chicago',period:'Not enough evidence',rarity:'4 / 10',source:'Bottle glass',notes:'Amber tone and vessel morphology point toward beverage, medicine, or household packaging, but the observed clues do not securely date the fragment.'},
        {key:'clear',id:'LM-040',name:'Clear / embossed',color:'#c8d8d4',provenance:'Montrose Beach · Chicago',period:'Undetermined',rarity:'1 / 10 color',source:'Marked container glass',notes:'The color is common, but embossing substantially raises historical interest and may support maker-level research.'},
        {key:'cobalt',id:'LM-039',name:'Cobalt',color:'#557ab6',provenance:'North Shore · Lake Michigan',period:'Undetermined',rarity:'5 / 10',source:'Medicine or decorative glass',notes:'Cobalt is less common on Lake Michigan and is often associated with medicine, poison, or decorative glass.'}
      ];
      ...
      async function restoreSavedSpecimens(){
        try{
          ...
          db.close();
          if(Array.isArray(rows)&&rows.length){savedSpecimens.splice(0,savedSpecimens.length,...rows);renderCollection();}
        }catch(err){}
      }
      ...
      function renderCollection(){
        ...
        savedSpecimens.forEach((d,index)=>{
          ...
          card.innerHTML=`<div class="spec-id">${d.id} · ${d.provenance.split(' · ')[0]}</div><div class="stone" style="background:linear-gradient(145deg,rgba(255,255,255,.55),${d.color} 68%,${d.color})"></div><h3>${d.name}</h3><p>${d.source} · ${d.period.toLowerCase()}</p>`;
          card.addEventListener('click',()=>openDetail(index));
          card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openDetail(index);}});
          grid.appendChild(card);
        });
        root.querySelector('#collectionCount').textContent=`${38+savedSpecimens.length} specimens`;
      }
      function openDetail(index){
        const d=savedSpecimens[index]; if(!d) return;
      ...
      function populateEditForm(){
        const d=savedSpecimens[activeDetailIndex]; if(!d) return;
      ...
      root.querySelector('#exportCollectionBtn').addEventListener('click',()=>{
        const payload={app:'Lakeglass',version:1,exportedAt:new Date().toISOString(),specimens:savedSpecimens};
      ...
      importInput.addEventListener('change',()=>{
        ...
        reader.addEventListener('load',()=>{
          try{
            ...
            const clean=rows.slice(0,500).filter(d=>d&&typeof d==='object').map((d,i)=>({
            ...
            renderCollection();
            show('collection');
          }catch(err){window.alert('That file is not a valid Lakeglass collection export.');}
          importInput.value='';
        });
